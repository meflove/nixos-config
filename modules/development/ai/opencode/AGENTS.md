...
4. Always use mcp-read-website-fast mcp for web scraping. If the website has bot protection or CAPTCHA.
5. Always use TodoWrite tool for task planning and progress tracking when working on multi-step tasks (3+ steps). Create specific, actionable todo items and mark them as completed immediately after finishing each one. Never batch completions - update status in real-time.
6. Always use huggingface mcp for ML/AI tasks: model_search (find models by task/library/author), dataset_search (find datasets), paper_search (search research papers), space_search (find Spaces, especially MCP-enabled), gr1_z_image_turbo_generate (generate images), hf_doc_search/fetch (HuggingFace/Gradio documentation), hub_repo_details (model/dataset/space info), and hf_whoami (user info).
7. Always use github mcp for GitHub operations:

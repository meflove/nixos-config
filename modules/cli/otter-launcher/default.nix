    ...
    nixosModules.${baseNameOf ./.} = {pkgs, ...}: {
      hm = {
        ...
        programs.otter-launcher = {
          ...
          #   tput_color = arg:
          #     builtins.readFile (
          #       pkgs.runCommand "tput-color-${toString arg}" {
          #         buildInputs = [pkgs.ncurses];
          #       } ''
          #         tput setaf ${toString arg}> $out
          #       ''
          #     );
          # in ''
          #   [general]
          ...
          #   exec_cmd = "/usr/bin/env bash -c"    # The exec command of your shell, default to sh
          ...
          #   header = "  ${tput_color 4} ${tput_color 7} otter-launcher ${tput_color 4}>${tput_color 7} "
          #   header_cmd_trimmed_lines = 1
          #   list_prefix = "     "
          #   selection_prefix = "   ${tput_color "1 bold"}> "
          #   place_holder = "type and search"
          #   suggestion_mode = "list"
          #   suggestion_lines = 5
          #   prefix_padding = 3
          #   prefix_color = "${tput_color 3}"
          #   description_color = "${tput_color 8}"
          #   place_holder_color = "${tput_color 60}"
          #   hint_color = "${tput_color 60}"
          #
          #
          ...
          #   cmd = """
          #   desktop_file() {
          #       fd '^.*\\.desktop$' --follow $HOME/.local/share/flatpak/exports/share/applications 2>/dev/null
          #       fd '^.*\\.desktop$' --follow /run/current-system/sw/share/applications/ 2>/dev/null
          #       fd '^.*\\.desktop$' --follow $HOME/.nix-profile/share/applications 2>/dev/null
          #       fd '^.*\\.desktop$' --follow /etc/profiles/per-user/angeldust/
          #   }
          #
          #   temp_file=$(mktemp)
          #
          ...
          #
          #   niri msg action spawn-sh -- "gtk-launch "$(basename "$selected_path")""
          #   """
          #
          #   [[modules]]
          ...
          #   prefix = "sh"
          #   cmd = "hyprctl dispatch exec 'ghostty -e {}' || niri msg action spawn-sh -- 'ghostty -e {}'"
          #   with_argument = true
          #
          #
          #   [[modules]]

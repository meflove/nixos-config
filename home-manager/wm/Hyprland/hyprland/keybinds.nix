...
bind = Alt+Super, T, exec, ~/.config/ags/scripts/color_generation/switchwall.sh # Change wallpaper
##! Actions
...
bind = ,Print, exec, ~/.config/ags/scripts/grimblast.sh --freeze copy area # Screen snip
bind = Super+Shift+Alt, S, exec, grim -g "$(slurp)" - | swappy -f - # Screen snip >> edit
# OCR
bind = Super+Ctrl,T,exec,grim -g "$(slurp $SLURP_ARGS)" "tmp.png" && tesseract -l eng "tmp.png" - | wl-copy && rm "tmp.png" # Screen snip to text >> clipboard
...
bindl= Ctrl+Shift,Print, exec, mkdir -p ~/Pictures/Screenshots && ~/.config/ags/scripts/grimblast.sh copysave screen ~/Pictures/Screenshots/Screenshot_"$(date '+%Y-%m-%d_%H.%M.%S')".png # Screenshot >> clipboard & file
# Recording stuff
...
bind = Super, C, exec, kitty nvim # Launch NeoVim (editor)
bind = Super, E, exec, nautilus --new-window # Launch Nautilus (file manager)
bind = Super+Alt, E, exec, thunar # [hidden]
bind = Super, W, exec, zen-twilight# [hidden] Let's not give people (more) reason to shit on my rice
...
bind = Super+Shift, T, exec, ayugram-desktop # Launch Ayugram
bind = Super+Shift, D, exec, equibop # Launch Discord
bind = Super, S, exec, spotify-launcher # Launch Spotify
bind = Super, K, exec, kitty -e calcurse # Launch calendar
